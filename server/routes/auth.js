...
router.get('/me', authenticateToken, async (req, res) => {
    try {
        const user = await getOne(
            'SELECT id, name, email, role, subjects, credits, plan, billing_period_start, billing_period_end, created_at, last_login, token_balance, role_admin FROM users WHERE id = ?',
            [req.user.userId]
        );
